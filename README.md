...
Edge dynamics run every 5 ticks from tick 160 onward. For each non-corner node: if it has exactly one neighbor, it moves to the midpoint between itself and that neighbor; if the resulting distance is below 256, it is kicked away from that neighbor just enough to reach 256 (plus a small 2.0 buffer), then it reconnects to all other nodes. If it has more than one neighbor, it deletes the edge to the neighbor with the highest chip count, breaking ties by closest Euclidean distance. When a tie occurs — another neighbor (still in the graph after this deletion) shares the victim's chip count — the mutation block runs: the node adopts the victim's full parameter set, keeps one randomly chosen parameter from its own previous value, and mutates one randomly chosen parameter by ±0.1; all six parameters are then clamped to [0.5, 8.0]. If any node moved, the parameter grids are recomputed.
